...
def register_child_os_api(app, db_path, api_token=None):
    ...
    def child_os_api_family(parent_id):
        ...
        return response({"family": dict(parent), "students": [dict(row) for row in students]})
    ...
    def child_os_api_schedule(student_name):
        ...
        conn.close()
        return response({"student_name": student_name, "schedule": [dict(row) for row in rows]})
    ...
    def child_os_api_credits(student_name):
        ...
        conn.close()
        return response({"student_name": student_name, "credits": dict(row) if row else None})
    ...
    def child_os_api_invoices(student_name):
        ...
        conn.close()
        return response({"student_name": student_name, "invoices": [dict(row) for row in rows]})
    ...
    def child_os_api_reschedule_options():
        ...
        conn.close()
        return response({"student_name": student_name, "lessons": [dict(row) for row in rows]})
